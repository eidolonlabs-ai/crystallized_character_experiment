...
└── baseline/
    ├── base_split/               (~19 examples; raw split)
    │   ├── train.jsonl
    │   └── valid.jsonl
    ├── base_split_512/            (truncated to 512 tokens)
    ├── base_split_768/            (truncated to 768 tokens)
    ├── base_split_2048/           (untruncated; all examples fit)
    ├── augmented_split/          (~319 examples; real + synthetic)
    ├── augmented_split_512/, augmented_split_768/, augmented_split_2048/
    ├── augmented_curated_split/  (only created if augmented_curated.jsonl is generated by prepare_all_datasets.sh)
...
- **Standard variant**: MAX_SEQ_LENGTH = 2048 tokens (all examples fit without truncation)
- **Deep variant**: MAX_SEQ_LENGTH = 2048 tokens (all examples fit without truncation)
